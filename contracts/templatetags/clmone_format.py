...
def brand_name(value):
    """Render legacy immutable records with the current product name."""
    if value is None:
        return ''
    old_name = 'Doc' + 'Clad'
    return str(value).replace(old_name, 'CLM One').replace(old_name.lower(), 'clmone')
...
# underlying --status-*-fg/--status-*-bg tokens in clmone-tokens.css, so
