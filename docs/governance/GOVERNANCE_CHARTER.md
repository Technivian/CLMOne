...
CMS Aegis references are retained only in migration notes and traceability records.
...
Theme parity (pilot amendment):

- The authenticated pilot operates **light-first**. Semantic components must
  define complete light-theme states using shared tokens.
- Full dark-theme parity is **deferred post-pilot** and must not block pilot
  stabilisation work. Do not introduce dark-mode scope during Sprint 0.
