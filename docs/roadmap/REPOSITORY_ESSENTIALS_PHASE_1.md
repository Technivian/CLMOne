...
| Microsoft SSO | **Partial — hardened implementation and activation preflight; activation blocked** | Password auth, optional OIDC settings, SAML endpoints, SCIM/group support, MFA handling, and identity audit remain. A default-off Microsoft Entra mode adds exact tenant/issuer validation, tenant-specific discovery configuration, explicit domain allowlisting, pre-provisioned active OIDC-workspace membership, no JIT account/role creation, provider-specific audit evidence, explicit login presentation, and a read-only boolean-only activation preflight. | No named tenant, app registration, vaulted credential, redirect registration, or environment is configured. Non-production operator activation and production identity integration remain separately authorized; forced SSO, password retirement, SCIM authority, and role/group mapping are excluded. |
