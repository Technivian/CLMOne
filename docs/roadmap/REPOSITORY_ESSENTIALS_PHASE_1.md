...
| Document ingestion security | **Implementation prepared; acceptance pending** | Proposed ADR-0016 now has a default-off quarantine implementation: a technical tenant-scoped attempt record, separate private storage, byte-signature and DOCX archive checks, provider-neutral verdicts, ClamAV adapter, explicit clean-only canonical release, append-only evidence, abort control, and legal-hold-aware retention. | ADR-0016 still requires explicit acceptance and green exact-SHA review. Named scanner/storage infrastructure, retention approval, operator observation, deployed activation and real-data use remain unauthorized. |
...
| 2a | Quarantine-first document ingestion | Product + Engineering + Security | **Implementation prepared; ADR acceptance pending** | Explicit ADR-0016 acceptance, named scanner/private storage, retention policy, exact-SHA CI and operator authority | Default-off/no-document-before-clean, malformed/archive, malicious/outage, isolation, audit, explicit release, cleanup, retention and preflight tests | High | **Code and synthetic tests prepared default off; no activation, credentials, real data, OCR/AI or external upload authorized** |
