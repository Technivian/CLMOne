...
| 7 | Microsoft SSO | Product + Engineering + Security | **Implementation in progress; activation blocked** | Owner-directed authentication priority; named Entra tenant, secret-store configuration, and identity release authority | Tenant/issuer/domain, pre-provisioning, duplicate/inactive identity, audit, password fallback, MFA/session, and rollback evidence | High | **Single-tenant pre-provisioned Entra mode implemented default off; no credentials or activation** |
