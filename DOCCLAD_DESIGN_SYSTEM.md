...
DocClad currently reads as "default Tailwind SaaS" because it *is* default Tailwind SaaS:

| Generic tell (verified across the category) | DocClad today |
|---|---|
...
product to DocClad's matter/billing/IOLTA scope) hides trust accounting behind
reassurance copy; Ironclad owns "editorial cream + working green." DocClad's actual
...
## 4) Tokens (summary — canonical values in docclad-tokens.css)
...
(`contracts/templatetags/docclad_format.py`) — templates never pick colors.
...
## 5) Signature elements (what makes it DocClad, not generic)
