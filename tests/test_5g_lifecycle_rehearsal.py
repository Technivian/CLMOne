...
class TransitionGraph(TestCase):
    ...
    def test_terminal_states_cannot_reopen(self):
        for term in ('EXPIRED', 'TERMINATED', 'COMPLETED', 'CANCELLED'):
            c = _contract(self.org, self.user, term)
            for to in ('DRAFT', 'ACTIVE', 'IN_REVIEW'):
                with self.assertRaises(InvalidContractTransition):
                    self.svc.transition(c, to, self.user)

    def test_same_status_is_idempotent_noop(self):
        c = _contract(self.org, self.user, 'IN_REVIEW')
        before = AuditLog.objects.count()
        self.svc.transition(c, 'IN_REVIEW', self.user)  # no error
...
class AuthorizationAndAudit(TestCase):
    ...
    def test_unauthenticated_actor_rejected(self):
        c = _contract(self.org_a, self.owner, 'DRAFT')
        with self.assertRaises(ContractTransitionForbidden):
            self.svc.transition(c, 'IN_REVIEW', actor=None)  # not system
    ...
    def test_one_chained_audit_per_success(self):
        c = _contract(self.org_a, self.owner, 'DRAFT')
        self.svc.transition(c, 'IN_REVIEW', self.owner)
...
class ApprovalPrerequisite(TestCase):
    ...
    def test_no_approval_blocks(self):
        c = _contract(self.org, self.user, 'APPROVED')
        with self.assertRaises(ContractTransitionPreconditionFailed):
            self._activate(c)

    def test_pending_approval_blocks(self):
        c = _contract(self.org, self.user, 'APPROVED'); _approve(c, 'PENDING')
        with self.assertRaises(ContractTransitionPreconditionFailed):
            self._activate(c)

    def test_rejected_approval_blocks(self):
        c = _contract(self.org, self.user, 'APPROVED'); _approve(c, 'REJECTED')
        with self.assertRaises(ContractTransitionPreconditionFailed):
            self._activate(c)

    def test_approved_request_allows(self):
        c = _contract(self.org, self.user, 'APPROVED'); _approve(c, 'APPROVED')
    ...
    def test_approval_of_other_contract_does_not_satisfy(self):
        other_c = _contract(self.org, self.user, 'APPROVED'); _approve(other_c, 'APPROVED')
        c = _contract(self.org, self.user, 'APPROVED')  # no approval of its own
        with self.assertRaises(ContractTransitionPreconditionFailed):
            self._activate(c)
    ...
    def test_approval_in_other_org_does_not_satisfy(self):
        ...
        oc = _contract(self.other, self.other_u, 'APPROVED'); _approve(oc, 'APPROVED')
        with self.assertRaises(ContractTransitionPreconditionFailed):
            self._activate(c)
...
class SignaturePrerequisite(TestCase):
    ...
    def _ready(self):
        c = _contract(self.org, self.user, 'APPROVED'); _approve(c, 'APPROVED'); return c
    ...
    def test_other_contract_signature_does_not_block(self):
        c = self._ready()
        other = _contract(self.org, self.user, 'APPROVED')
...
class BypassPaths(TestCase):
    ...
    def test_bulk_api_valid_transition_goes_through_service(self):
        ...
        c.refresh_from_db(); self.assertEqual(c.status, 'IN_REVIEW')
...
class LifecycleConcurrencyPostgres(TransactionTestCase):
    ...
    def test_duplicate_activation_commits_once(self):
        ...
        c = _contract(org, u, 'APPROVED'); _approve(c, 'APPROVED')
    ...
    def test_racing_terminal_transitions_one_winner(self):
        ...
        c.refresh_from_db()
        self.assertIn(c.status, ('EXPIRED', 'COMPLETED'))  # exactly one valid terminal
