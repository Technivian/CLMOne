...
class DesignSystemPhaseTwoATests(SimpleTestCase):
    ...
    def test_phase_three_b_standard_lists_use_the_shell_header_and_scaffold(self):
        ...
        expectations = {
            'repository.html': ('authenticated_page_title', 'dc-ds-list-page', 'dc-ds-list-toolbar', 'dc-ds-list-tabs'),
            ...
            'approval_request_list.html': ('authenticated_page_title', 'dc-ds-list-page', 'dc-ds-list-header', 'dc-ds-list-tabs'),
            'approval_rule_list.html': ('authenticated_page_title', 'flow=True', 'scaffold_class_name="dc-ds-list-page"'),
        }
        for template_name, required in expectations.items():
            content = (templates / template_name).read_text()
            for value in required:
