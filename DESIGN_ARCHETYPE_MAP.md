...
|theme/templates/contracts/organization_team.html|NetworkPage|WorkspacePage|Medium|Team member management with table + invite + destructive actions|**Batch 5 Step 5 discovery (2026-05-18)** — 48 raw Tailwind hits, multiple inline role/revoke/deactivate/reactivate forms, complex multi-panel layout, permission conditionals. Slice B — defer until Slice A validated.|High|High|Defer to Slice B; normalize raw Tailwind in multiple member-action panels|
...
|theme/templates/profile.html|CommandPage|WorkspacePage|High|User profile + MFA enrollment + recovery codes — multi-panel WorkspacePage|**Batch 5 Step 5 discovery (2026-05-18)** — 20 raw Tailwind hits; MFA enrollment flow with `name="action"` multi-submit buttons, recovery codes, `mfa_required` conditional; HIGH security risk. Defer indefinitely.|High|High|Defer — MFA/security-critical; multiple named submit actions; security-sensitive conditionals|
